...
if (TRUE === isset($_POST['login'])) {
	...
	if ($_POST['vercode'] != $_SESSION['vercode']) {
		...
		echo "<script>alert('Code de vérification incorrect')</script>";
			...
			// On construit la requete SQL pour recuperer l'id, le readerId et l'email du lecteur à partir des deux variables ci-dessus
			// dans la table tblreaders

			if(!empty($mail) && !empty($pass)
			&& strlen($mail) <= 40
			&& preg_match("#^[A-Za-z]+@{1}[A-Za-z]+\.{1}[A-Za-z]{2,}$#", $mail)
			...
			<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 offset-md-3">
				<form method="post" action="index.php">
					<div class="form-group">
						...
						<input type="text" name="emailid" pattern="^[A-Za-z]+@{1}[A-Za-z]+\.{1}[A-Za-z]{2,}$" maxlength="40" required>
					</div>

					<div class="form-group">
						<label>Entrez votre mot de passe</label>
						<input type="password" name="password" pattern="^[A-Za-z0-9.+_@!?&§%]+$" maxlength="20" required>
						<p>
